# Comprehensive Google Ads Performance Analysis with Python
# This project simulates Google Ads data at campaign, ad group, keyword, and demographic levels,
# performs detailed performance analysis, anomaly detection, and generates reports.
# It now includes data visualization (graphs) to showcase analytical and presentation skills.
# This script aims to demonstrate advanced Python capabilities in data manipulation,
# statistical analysis, reporting, and visualization, relevant for digital marketing and data analytics roles.
                        ...
                        for gender in genders:
                            for location in locations:
                                ...
                                if is_global_anomaly_day or np.random.rand() < ANOMALY_PROBABILITY:
                                    clicks, conversions, cost = apply_anomaly(clicks, conversions, cost,
                                                                              ANOMALY_MAGNITUDE_FACTOR)
                                ...
                                all_data.append({
                                    ...
                                    'Simulated Revenue': round(conversions * np.random.uniform(50, 200), 2)
...
def calculate_metrics(df):
    ...
    df['CPA'] = (df['Cost'] / df['Conversions'].replace(0, np.nan)).replace([np.inf, -np.inf], np.nan).fillna(
        0)  # Cost Per Acquisition
    df['ROAS'] = (df['Simulated Revenue'] / df['Cost'].replace(0, np.nan)).replace([np.inf, -np.inf], np.nan).fillna(
...
def calculate_period_over_period_change(df, period_type='week'):
    ...
    for col in ['Clicks', 'Cost', 'Conversions', 'CTR', 'CPA', 'ROAS']:
        ...
        period_agg[f'{col}_Change_Pct'] = period_agg[f'{col}_Change_Pct'].fillna(
            0)  # Fill remaining NaNs (e.g., 0/0) with 0
...
def generate_optimization_insights(campaign_perf_df, daily_anomalies_df, demographic_perf_age, demographic_perf_gender):
    ...
    if avg_cpa > 50:  # Example threshold
        insights.append(
            "- Recommendation: Review campaigns with high CPA. Consider refining targeting or improving ad copy.")
    if avg_roas < 2.0:  # Example threshold
        insights.append(
            "- Recommendation: Focus on improving ROAS for underperforming campaigns. Look into conversion tracking accuracy.")
    ...
    for _, row in campaign_perf_df.iterrows():
        if row['CPA'] > 70 and row['Conversions'] > 0:
            insights.append(
                f"- Campaign '{row['Campaign Name']}': High CPA (${row['CPA']:.2f}). Investigate keyword bids or ad group performance.")
        if row['ROAS'] < 1.5 and row['Simulated Revenue'] > 0:
            insights.append(
                f"- Campaign '{row['Campaign Name']}': Low ROAS ({row['ROAS']:.2f}x). Consider pausing underperforming keywords/ad groups.")
        if row['CTR'] < 1.0:
            insights.append(
                f"- Campaign '{row['Campaign Name']}': Low CTR ({row['CTR']:.2f}%). Improve ad relevance or expand keyword coverage.")
    ...
    if not daily_anomalies_df.empty:
        ...
        for _, row in daily_anomalies_df.iterrows():
            ...
            insights.append(
                f"- On {row['Date'].strftime('%Y-%m-%d')}: Significant drop in {metric_impacted}. Investigate external factors or account changes.")
    else:
    ...
    insights.append(
        f"- Best performing Age Group by Conversion Rate: '{best_age_group['Age Group']}' ({best_age_group['Conversion Rate']:.2f}%)")
    ...
    if not demographic_perf_gender[demographic_perf_gender['Conversions'] > 0].empty:
        worst_gender_cpa = demographic_perf_gender[demographic_perf_gender['Conversions'] > 0].loc[
            demographic_perf_gender[demographic_perf_gender['Conversions'] > 0]['CPA'].idxmax()]
        insights.append(
            f"- Consider reviewing targeting for Gender '{worst_gender_cpa['Gender']}' due to high CPA (${worst_gender_cpa['CPA']:.2f}).")
    else:
...
def create_and_save_plot(df, x_col, y_col, title, filename, plot_type='line', hue_col=None, y_label=None, x_label=None,
                         rotation=0):
    """
...
def generate_full_report(df):
    ...
    create_and_save_plot(daily_performance_df, 'Date', 'Clicks', 'Daily Clicks Trend', 'daily_clicks_trend.png',
                         y_label='Total Clicks')
    create_and_save_plot(daily_performance_df, 'Date', 'Conversions', 'Daily Conversions Trend',
                         'daily_conversions_trend.png', y_label='Total Conversions')
    create_and_save_plot(daily_performance_df, 'Date', 'Cost', 'Daily Cost Trend', 'daily_cost_trend.png',
                         y_label='Total Cost ($)')
    ...
    create_and_save_plot(campaign_performance_df.sort_values(by='Conversions', ascending=False).head(5),
                         'Campaign Name', 'Conversions', 'Top 5 Campaigns by Conversions',
                         'top_campaigns_conversions.png',
    ...
    print(wow_performance_df[['Period', 'Clicks', 'Clicks_Change_Pct', 'Conversions', 'Conversions_Change_Pct', 'Cost',
                              'Cost_Change_Pct', 'ROAS', 'ROAS_Change_Pct']].tail(3).round(2).to_string())

    # 8.6 Anomaly Detection Report
    print("\n--- Anomaly Detection Report ---")
    clicks_anomalies = detect_anomalies(daily_performance_df, 'Clicks', ANOMALY_DETECTION_WINDOW_SIZE,
                                        Z_SCORE_THRESHOLD)
    conversions_anomalies = detect_anomalies(daily_performance_df, 'Conversions', ANOMALY_DETECTION_WINDOW_SIZE,
                                             Z_SCORE_THRESHOLD)

    all_anomalies = pd.concat([clicks_anomalies, conversions_anomalies]).drop_duplicates(subset=['Date']).sort_values(
        by='Date')
    ...
    if not all_anomalies.empty:
        ...
        for _, row in all_anomalies.iterrows():
            ...
            print(
                f"- Date: {row['Date'].strftime('%Y-%m-%d')}, Metric: {metric_impacted}, Value: {row[metric_impacted]:,.0f}, Z-Score: {row[f'{metric_impacted}_Z_Score']:.2f}")
    else:
    ...
    if not budget_pacing_df.empty:
        ...
        print(budget_pacing_df[['Date', 'Cumulative Cost', 'Expected Cumulative Budget', 'Budget Variance']].tail(
            7).round(2).to_string())
    ...
    create_and_save_plot(demographic_perf_age, 'Age Group', 'Conversions', 'Conversions by Age Group',
                         'conversions_by_age.png', plot_type='bar')
    create_and_save_plot(demographic_perf_gender, 'Gender', 'Conversions', 'Conversions by Gender',
                         'conversions_by_gender.png', plot_type='bar')
    create_and_save_plot(demographic_perf_location, 'Location', 'Conversions', 'Conversions by Location',
                         'conversions_by_location.png', plot_type='bar')
    create_and_save_plot(demographic_perf_age, 'Age Group', 'CPA', 'CPA by Age Group', 'cpa_by_age.png',
                         plot_type='bar', y_label='Cost Per Acquisition ($)')

    # 8.9 Optimization Insights
    insights_report = generate_optimization_insights(campaign_performance_df, all_anomalies, demographic_perf_age,
                                                     demographic_perf_gender)
...
if __name__ == "__main__":
    ...
    daily_perf, campaign_perf, ad_group_perf, keyword_perf, wow_perf, anomalies_df, budget_pacing_df, demographic_perf_age, demographic_perf_gender, demographic_perf_location = generate_full_report(
        ads_df)
